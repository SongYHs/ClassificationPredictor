...
if __name__ =="__main__":
    ...
    server_model = TritonModel(config.model_name)
    ...
    port_size = int(inputs.shape[0]/len(server_ports))
    max_size = int(args.max_size/len(server_ports)) if args.max_size else port_size
    ...
    for i,server_port in enumerate(server_ports):
        inputs0 = inputs[i*port_size: min([i*port_size+ port_size, inputs.shape[0]])]
        tags0 = tags[i*port_size: min([i*port_size + port_size, inputs.shape[0]])]
