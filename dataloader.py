...
def fun_ngram(ngram, pad_size, msg):
    # msg = re.sub("[^\u4e00-\u9fa5]+",' ', msg)
    if pad_size:
        length = min([len(msg),pad_size])
    else:
        length = len(msg)
    return [msg[i:i+j+1] for j in range(ngram) for i in range(length-j)]

def word_ngram(ngram, pad_size):
    return partial(fun_ngram, ngram,pad_size)
...
class DataGenerator:
    def __init__(self, config):# , data_dir,  sep='\t', ngram=1) train_ratio val_ratio test_ratio:
        ...
        self.tokenizer = word_ngram(self.ngram, self.pad_size)
    ...
    def load_dataset(self, path, need_seq_len=False):
        ...
        with open(path, 'r', encoding='utf-8') as f:
            for line in tqdm(f):
                ...
                tokenV = []
                seq_len = len(token)/self.ngram
                if self.pad_size:
                    if seq_len <= self.pad_size:
                        token.extend([PAD]*int((self.pad_size-seq_len)*self.ngram))
                    else:
                        raise "数据处理失败，文本长度大于实际长度"
                for word in token:
